...
export default function ProjectCard({ title, description, imageUrl, tags, isMaximized, onClick }: ProjectCardProps) {
    return (
        <div className={`flex bg-linear-to-br from-[#2c2c2c] to-[#242424] hover:bg-[#4e4e4e] rounded-2xl shadow-[5px_12px_30px_rgba(0,0,0,0.45)] border-l-4 border-[#eb6628]
            ...
            <div className={`relative flex flex-col justify-between h-full max-[800px]:py-2 max-[800px]:pr-2 ${!isMaximized ? 'w-[45%] py-4 min-[800px]:px-[2%] max-[801px]:px-[3%] max-[800px]:w-full' : 'w-[60%] py-[1.5%] px-[2%] max-[950px]:w-full'}`}>
                ...
                <div>
                    ...
                    <p className={`pb-2 ${!isMaximized ? '' : ''}`}>{description}</p>
                </div>

                <div className="flex gap-2 max-[1100px]:my-2 max-[800px]:gap-1 max-[800px]:gap-y-1 w-full max-[800px]:text-sm flex-wrap">
                    {tags.map((tag) => (
