...
export default function ProjectCard({ title, description, imageUrl, tags, isMaximized }: ProjectCardProps) {
    return (
        <div className={`flex bg-[#3d3d3d] hover:bg-[#4e4e4e] rounded-2xl shadow-[5px_12px_30px_rgba(0,0,0,0.45)] border-l-4 border-[#eb6628]
            ...
            ${!isMaximized ? 'w-full' : 'max-[950px]:flex-col max-[950px]:w-[90%]'}`}>
            <div className={`${!isMaximized ? 'w-[55%]' : 'w-[40%] max-[950px]:w-full'}`}>
                <img src={imageUrl} alt={title}
                    className={`aspect-video object-cover ${!isMaximized ? 'rounded-l-2xl' : 'max-[950px]:rounded-t-2xl min-[951px]:rounded-l-2xl'}`} />
            </div>

            <div className={`relative flex flex-col justify-between h-full max-[800px]:py-2 max-[800px]:pr-2 ${!isMaximized ? 'w-[45%] p-4' : 'w-[60%] py-[1.5%] px-[4%] max-[950px]:w-full'}`}>
                ...
                <div>
                    ...
                    <p className={`pb-2 ${!isMaximized ? 'max-[800px]:hidden' : 'max-[1100px]:text-md max-[800px]:'}`}>{description}</p>
                </div>
